...
class GroupManager {
    ...
    TIMGroupInviteMember(inviteMemberParams: InviteMemberParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupInviteMember(nodeStrigToCString(JSON.stringify(params)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
    ...
    TIMGroupDeleteMember(deleteMemberParams: DeleteMemberParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupDeleteMember(nodeStrigToCString(JSON.stringify(params)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
    ...
    TIMGroupModifyGroupInfo(modifyGroupParams: ModifyGroupParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupModifyGroupInfo(nodeStrigToCString(JSON.stringify(params)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
    ...
    TIMGroupGetMemberInfoList(getGroupMemberInfoParams: GetGroupMemberInfoParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupGetMemberInfoList(nodeStrigToCString(JSON.stringify(params)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
    ...
    TIMGroupModifyMemberInfo(modifyMemberInfoParams: ModifyMemberInfoParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupModifyMemberInfo(nodeStrigToCString(JSON.stringify(params)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
    ...
    TIMGroupGetPendencyList(getPendencyListParams: GetPendencyListParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupGetPendencyList(nodeStrigToCString(JSON.stringify(params)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
    ...
    TIMGroupHandlePendency(handlePendencyParams: HandlePendencyParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupHandlePendency(nodeStrigToCString(JSON.stringify(params)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
    ...
    TIMGroupSearchGroups(searchGroupsParams: SearchGroupParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupSearchGroups(nodeStrigToCString(JSON.stringify(searchParams)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
    ...
    TIMGroupSearchGroupMembers(searchMemberParams: SearchMemberParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupSearchGroupMembers(nodeStrigToCString(JSON.stringify(searchParams)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
    ...
    TIMGroupInitGroupAttributes(initAttributesParams: InitGroupAttributeParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupInitGroupAttributes(nodeStrigToCString(groupId), nodeStrigToCString(JSON.stringify(attributes)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
    ...
    TIMGroupSetGroupAttributes(params: InitGroupAttributeParams): Promise<commonResponse> {
        ...
        return new Promise((resolve, reject) => {
            const successCallback: CommonCallbackFun = (code, desc, json_param, user_data) => code === 0 ? resolve({ code, desc, json_param, user_data }) : reject(this.getErrorResponse({ code, desc }));
            const code = this._imskdLib.TIMGroupSetGroupAttributes(nodeStrigToCString(groupId), nodeStrigToCString(JSON.stringify(attributes)), jsFuncToFFIFun(successCallback), userData);
            if (code !== 0) reject(this.getErrorResponse({ code }));
        });
    }
